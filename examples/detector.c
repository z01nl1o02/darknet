...
void validate_detector_recall(char *cfgfile, char *weightfile, char* pathfile)
{
    network *net = load_network(cfgfile, weightfile, 0);
    set_batch_network(net, 1);
    fprintf(stderr, "Learning Rate: %g, Momentum: %g, Decay: %g\n", net->learning_rate, net->momentum, net->decay);
...
void test2_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *output_folder)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    float nms=.45;
    int image_idx = 0;
    for(image_idx = 0; image_idx < plist->size; image_idx++)
    {
        ...
        free_detections(dets, nboxes);
        if(output_folder){
            char outfile[1024], short_name[1024];
            char* pos = strrchr(input,'/');
            if(pos == NULL)
                sprintf(short_name,"%s", input);
            else
                sprintf(short_name,"%s", pos+1);
            sprintf(outfile,"%s/%s",output_folder,short_name);
            save_image(im, outfile);
        }
        else{
            save_image(im, "predictions");
...
void run_detector(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "test2")) test2_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, outfile);
    else if(0==strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear);
    else if(0==strcmp(argv[2], "valid")) validate_detector(datacfg, cfg, weights, outfile);
    else if(0==strcmp(argv[2], "valid2")) validate_detector_flip(datacfg, cfg, weights, outfile);
    else if(0==strcmp(argv[2], "recall")) validate_detector_recall(cfg, weights,NULL);
    else if(0==strcmp(argv[2], "recall2")) validate_detector_recall(cfg, weights,filename);
